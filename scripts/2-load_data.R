...
##
## Purpose of script: To load all the data files needed for the analysis
##
## Author: Yufan Gong
##
...

{
  ...
  list.dirs(here::here(),recursive = FALSE) %>%
    ...
    base::grep("noob",., 
         value=TRUE, ignore.case = TRUE) %>%
    keep(~str_detect(.x,"filter_ctrl|filter_pd")) %>%
    discard(~str_detect(.x,"old|archive|nors_filter|l_c|d_c")) %>%
    map(.,load,.GlobalEnv)
