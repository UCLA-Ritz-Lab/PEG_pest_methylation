---
title: "Copper pesticide exposure and DNA methylation"
author: "Yufan Gong"
...
This project aims to investigate the association between copper pesticide exposure and DNA methylation.
  ...
  - **Outcome**
    - Beta-matrix of DNA methylation
      ...
      - Created a residual matrix using `empiricalBayesLM` function in `WGCNA` package to remove the unwanted variation(e.g., age at blood draw, sex, race, smoking status, study wave, cell composition and OP counts)
      
  - **Questions left since last time** 
    - **Scatter plot: **
    ...
    - **Things to do**
      - According to the scatter plot, there is few difference between case and control in terms of DNA methylation level. Thus, we don't need to show the result of GSEA for case only.
      - Need to select the first 10 CpG sites solely significant in cases and do another scatter plot for these CpG sites.
      - Check the association between case and control methylation levels using the original beta values. (per 10/29/24 discussion, the beta values refers to the DMP estimate coefficient betas, not the beta values in the DNAm matrix)
      - Compare DMP models adjusted/not adjusted for PD status in the total study sample (PD could be a collider).
...
combined_resid_win_filter_total <- list(combined_resid_win_filter_case, combined_resid_win_filter_ctrl) %>% 
  bind_cols()
...
## CpG level analysis
  ...
  - This step regressed individual CpG methylation level to the count measurement using the `ChAMP` package. 
...
##### Total (PD patients and no-PD controls) (N= 806)
![manhatton](figures/dmp_plot_total.png)


A total of 140 CpG sites were identified as differentially methylated with copper exposure in our study sample when adjusted for OP. Another 41 CpGs were also identified solely in PD patients. The top 10 CpG sites are shown in the figure below. The x-axis represents the copper count, and the y-axis represents the beta value of CpG sites. We can see that the beta value of PD patients were consistently lower than controls across the top 10 CpG sites.



#### Scatter plot for the top 10 CpG sites (Copper count vs. beta value)

![scatter_total](figures/scatter_plot_residual_total.png)

![scatter_solely_case](figures/scatter_plot_residual_sole_case.png)

#### Scatter plot for methylation level between case and control

![scatter_correlation_original](figures/beta_correlation_plot_original.png)

![scatter_correlation_adjusted](figures/beta_correlation_plot_adjusted.png)

#### EWAS table

The detailed information of the differentially methylated CpG sites were shown in the table below.

##### Total (PD patients and no-PD controls, without controlling for PD status) (N= 806)
```{r}

datatable(dmp_count_total$total %>% 
          ...
          caption = "Table: EWAS analysis in total study sample without controlling for PD status (n = 806)"
          )
```

##### Bicor association (PD patients and no-PD controls, without controlling for PD status) (N= 806)
...
bicor_combined <- list(
  ...
  list("p.value_total", "p.value_case", "p.value_ctrl"),
  list(bicor_total, bicor_case, bicor_ctrl),
  list("cor_total", "cor_case", "cor_ctrl")
) %>% 
  pmap(function(data1, data2, data3, data4){
    data1$total %>% 
      rownames_to_column("cpg") %>%
      filter(cpg %in% sig_cpg$cpg) %>%
      dplyr::rename(!!data2 := P.Value) %>% 
      select(cpg, !!data2) %>% 
      left_join(
        data3 %>% 
          rename_all(str_to_lower) %>%
          dplyr::rename(cpg = id, !!data4 := cor) %>% 
          select(cpg, !!data4), 
        by = "cpg"
      ) %>% 
      relocate(cpg, !!data4, !!data2)
  }) %>% 
  purrr::reduce(left_join, by = "cpg")

dmp_table_final <- sig_cpg %>% 
  dplyr::rename_all(str_to_lower) %>%
  dplyr::rename(pos = mapinfo,
         gene_region = feature,
         coefficient = b,
         ) %>% 
  select(cpg, chr, pos, gene, gene_region, probe_snps, probe_snps_10) %>% 
  left_join(bicor_combined, by = "cpg")


datatable(dmp_table_final, 
          rownames = FALSE,
          extensions = "Buttons",
          options = list(
            dom = "Bfrtip",
            ...
            scrollX = TRUE, 
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: Biweight midcorrelation in total study sample (n = 806)"
          )

```


##### Total (PD patients and no-PD controls, controlling for PD status) (N= 806)
```{r}
datatable(dmp_count_ctrlpd_total$total %>% 
            filter(-log10(P.Value) > 6) %>% 
            arrange(P.Value), 
          rownames = TRUE,
          extensions = "Buttons",
          options = list(
            ...
            scrollX = TRUE, 
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: EWAS analysis in total study sample controlling for PD status (n = 806)"
          )
```


##### 41 CpG sites identified in PD patients only
```{r}
datatable(dmp_count_case$total %>% 
            filter(-log10(P.Value) > 6) %>% 
            filter(rownames(.) %notin% 
                     rownames(dmp_count_total$total %>% 
                                filter(-log10(P.Value) > 6))) %>% 
            arrange(P.Value), 
          rownames = TRUE,
          extensions = "Buttons",
          options = list(
            ...
            scrollX = TRUE, 
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: 41 CpG sites identified in PD patients only"
          )
```
...
According to Dr. Paul's suggestion from last time, I put in all CpG sites in the `champ.gsea()` function and applied `gometh` method to identify the enriched GO terms and KEGG pathways. `gometh` method is a function in the `missMethyl` package that uses number of CpGs contained in genes to correct p-value. In addition, I also tried the newly developed `ebGSEA` [method](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6748733/) in the `ChAMP` [package](https://github.com/aet21/ebGSEA), which ranks genes instead of CpGs, according to the overall level of differential methylation. 
...
##### Total (PD patients and non-PD controls) (n = 806)

```{r}
datatable(gsea_champ_copper_total$DMP %>% 
            arrange(`P.DE`), 
          rownames = TRUE,
          extensions = "Buttons",
          options = list(
            ...
            scrollX = TRUE, 
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: Go term analysis in study sample (n = 806)"
          )
```
...
datatable(gsea_champ_copper_case$DMP %>%
            arrange(`P.DE`),
          rownames = TRUE,
          extensions = "Buttons",
          options = list(
            ...
            scrollX = TRUE,
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: Go term analysis in PD patients (n = 569)"
          )
```
...
Since `ChAMP` package does not provide `KEGG` pathway analysis, I used `missMethyl` package to do it.

```{r}
### method 2: missMethyl package
pacman::p_load(missMethyl, IlluminaHumanMethylation450kanno.ilmn12.hg19)


topCpGs_case <- dmp_count_case$total %>% 
  filter(-log10(P.Value) > 6) %>% 
  rownames_to_column("cpg") %>%
  arrange(P.Value)
...
datatable(gst_kegg_total %>% 
          ...
          options = list(
            ...
            scrollX = TRUE, 
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: KEGG pathway analysis in study sample (n = 806)"
          )
```
...
No significant KEGG pathway was identified in PD patients.

```{r}

# gst_kegg_case <- gometh(sig.cpg = sigCpGs_case_all, 
#                         all.cpg = rownames(PEG_NOOB_nors_win_filter_case), 
#                         collection = "KEGG", 
#                         plot.bias = FALSE)
# 
...
#                          # fixedHeader=TRUE,
#                          className = 'dt-center', targets = 0:4),
#           caption = "Table: KEGG pathway analysis in PD patients (n = 569)"
#           )
```

#### Empirical Bayes Gene Set Enrichment Analysis (ebGSEA)

##### Total (PD patients and non-PD controls) (n = 806)

```{r}

datatable(gsea_champ_copper_ebayes_total$GSEA[[1]] %>% 
            as.data.frame() %>%
            rownames_to_column("gene_set") %>%
            mutate(gene_set = str_replace_all(gene_set, "_", " "),
                   gene_set = str_to_lower(gene_set)) %>%
            arrange(`P(WT)`), 
          rownames = FALSE,
          extensions = "Buttons",
          options = list(
            ...
            scrollX = TRUE, 
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: Empirical bayes GSEA in study sample (n = 806)"
          )
```
...
```{r}
# datatable(gsea_champ_copper_ebayes_case$GSEA[[1]] %>% 
#             as.data.frame() %>%
#             rownames_to_column("gene_set") %>%
#             mutate(gene_set = str_replace_all(gene_set, "_", " "),
#                    gene_set = str_to_lower(gene_set)) %>%
...
#                          # fixedHeader=TRUE,
#                          className = 'dt-center', targets = 0:4),
#           caption = "Table: Empirical bayes GSEA in PD patients (n = 569)"
#           )
```

#### PATHER pathway analysis

```{r}
pacman::p_load(PANTHER.db, methylGSA)
...
datatable(methylglm_path_total_op, 
          rownames = TRUE,
          extensions = "Buttons",
          options = list(
            dom = "Bfrtip",
            ...
            scrollX = TRUE, 
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: PATHER pathway analysis in total study sample (n = 806)"
          )
...
## Region level analysis
  ...
  - `ChAMP` package was applied to identify DMRs in the total study sample (n = 806) and PD patients (n = 569).
  - FDR < 0.05 for suggestive association
  - **Three methods**
    - `Bumphunter`: It scans the genome to detect regions with subtle but consistent differences across multiple CpGs.
    - `DMRcate`: It uses a Gaussian kernel to smooth the p-values and identify regions with differential methylation. However, somehow it doesn't work (I'm still trying to figure out).
    - `ProbeLasso`: It uses a Lasso regression model to identify regions with differential methylation. However, it does not allow pheno which contains more than 2 levels and the `compare.group` parameter must be specified. Therefore, I did not use this method in the analysis.
...
To examine if my code for DMRcate is correct, I referred to the source code of `ChAMP` package and follow their steps to run the DMRcate method. However, there is no significant DMR identified when I looked at the total study sample (n = 806) but there are over 1800 DMRs. I think the `pheno` parameter in their model is a two-level variable, which is not suitable for our study. Therefore, I will use the `Bumphunter` method to identify DMRs in the total study sample and PD patients.

![souce_code](figures/source_code_dmrcate.png)

![mycode](figures/mycode_dmrcate.png)
...
```{r}
dmr_table_total <- dmr_bumphunter_champ_copper_total$total$BumphunterDMR %>% 
  arrange(p.valueArea)

datatable(dmr_table_total, 
          rownames = TRUE,
          extensions = "Buttons",
          options = list(
            dom = "Bfrtip",
            ...
            scrollX = TRUE, 
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: DMR in total study sample (n = 806)"
          )
```
...
```{r}
dmr_table_case <- dmr_bumphunter_champ_copper_case$total$BumphunterDMR %>%
  arrange(p.valueArea)

datatable(dmr_table_case,
          rownames = TRUE,
          extensions = "Buttons",
          options = list(
            dom = "Bfrtip",
            ...
            scrollX = TRUE,
                         ...
                         # fixedHeader=TRUE,
                         className = 'dt-center', targets = 0:4),
          caption = "Table: DMR in PD patients (n = 569)"
          )
```
...
Same as CpG level analysis, I used `gometh` method in `ChAMP` and `missMethyl` package to identify the enriched GO terms and KEGG pathways for DMRs.
...
```{r}
datatable(gsea_champ_copper_total$DMR %>% 
            arrange(`P.DE`), 
          rownames = TRUE,
          extensions = "Buttons",
          options = list(
...
datatable(gsea_champ_copper_case$DMR %>%
            arrange(`P.DE`),
          rownames = TRUE,
          extensions = "Buttons",
          options = list(
