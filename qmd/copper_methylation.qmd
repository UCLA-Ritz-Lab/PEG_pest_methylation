...
![scatter](figures/beta_correlation_plot.png)
...
According to Dr. Paul's suggestion from last time, I put in all CpG sites in the `champ.gsea()` function and applied `gometh` method to identify the enriched GO terms and KEGG pathways. `gometh` method is a function in the `missMethyl` package that uses number of CpGs contained in genes to correct p-value. 
...
## Region level analysis
  ...
  - **Three methods**
    ...
    - `DMRcate`: It uses a Gaussian kernel to smooth the p-values and identify regions with differential methylation. Somehow it doesn't work (still trying to figure out).
